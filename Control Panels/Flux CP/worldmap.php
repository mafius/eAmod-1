...
?>
	...
	<html>
		<head>
			...
			<title>
				Servidor Evangelis Ragnarok - High Rates Premium
			</title>
...
<?php
	...
	while ($char = $mysql->fetcharray($result))
	{
		$map = $char['last_map'];
		if (isset($maps[$map]))
		{ // Mapa configurado en coordenadas
			echo '
				<img src="' . (($char['sex'] == 'M')?'allia':'horde') . '.gif" style="position:absolute; border:0px; left:' . rand($maps[$map]['x1'], $maps[$map]['x2']) .'px; top: ' . rand($maps[$map]['y1'], $maps[$map]['y2']) .'px;" onMouseMove="show_tip(\'' . htmlformat($char['name']) . '\',\'' . htmlformat($char['gname']) . '\',\'' . $jobs[$char['class']] . '\',\'(' . $char['base_level'] . '/' . $char['job_level'] . ')\',\'' . htmlformat($char['last_map']) . '\',\'' . $char['class'] . '\');" onMouseOut="hide_tip();">
			';
		}
	}
	...
	if( $vid )
		echo '
					<center><b><font color="White">
						Created by Perseus<br>
						WorldMap Image by Gravity<br>
						Evangelis Ragnarok<br>
						2007-2008<br><br>
					</font></b></center>
				</body>
			</html>
