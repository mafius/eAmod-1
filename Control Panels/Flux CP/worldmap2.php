<?php
	...
	$options = array (
		'DBHost'		=> '192.168.0.4',
		'DBUser'		=> 'gaiaroreborn',
		'DBPass'		=> '1234andjusticeforall5678',
		'DBName'		=> 'gaia_main',
		'ServerName'	=> 'My Ragnarok Private Server',
		'ServerURL'		=> 'http://my.websyte.com'
	);
...
<?php
	...
	while( $char = $sql->fetcharray($data) )
	{
		$map = $char['last_map'];
		if( isset($maps[$map]) )
		{
			echo '<img src="' . $char['sex'] . '.gif" style="position:absolute; border:0px; left:' . rand($maps[$map]['x1'], $maps[$map]['x2']) .'px; top: ' . rand($maps[$map]['y1'], $maps[$map]['y2']) .'px;" onMouseMove="show_tip(\'' . htmlformat($char['name']) . '\',\'' . htmlformat($char['gname']) . '\',\'' . $jobs[$char['class']] . '\',\'(' . $char['base_level'] . '/' . $char['job_level'] . ')\',\'' . htmlformat($char['last_map']) . '\',\'' . $char['class'] . '\');" onMouseOut="hide_tip();">';
		}
	}
?>
...
<?php
	echo $options['ServerName'] . '<br><a href="' . $options['ServerURL'] . '">' . $options['ServerURL'] . '</a><br>';
?>
