<?php
	...
	if (!isset($_GET['opt']))
	{
		...
		opentable('Ranking Acumulativo Evento PVP');
	...
	DEFINE('PK_LADDER_ALL', "
		...
		WHERE
			`login`.`level` < '1' AND `login`.`state` = '0' AND `char_pvp`.`score` > 0
		ORDER BY
			`char_pvp`.`score` DESC,
	...
	DEFINE('PK_LADDER_JOB', "
		...
		FROM
			`char_pvp` LEFT JOIN `char` ON `char`.`char_id` = `char_pvp`.`char_id` LEFT JOIN `login` ON `login`.`account_id` = `char`.`account_id` LEFT JOIN `guild` ON `guild`.`guild_id` = `char`.`guild_id`
		WHERE
			`login`.`level` < '1' AND `login`.`state` = '0' AND `char`.`class` = '%d' AND `char_pvp`.`score` > 0
		ORDER BY
			`char_pvp`.`score` DESC,
			`char`.`base_level` DESC,
			`char`.`base_exp` DESC,
	...
	DEFINE('PK_LADDER_LKPA', "
		...
		FROM
			`char_pvp` LEFT JOIN `char` ON `char`.`char_id` = `char_pvp`.`char_id` LEFT JOIN `login` ON `login`.`account_id` = `char`.`account_id` LEFT JOIN `guild` ON `guild`.`guild_id` = `char`.`guild_id`
		WHERE
			`login`.`level` < '1' AND `login`.`state` = '0' AND (`char`.`class` = '%d' OR `char`.`class` = '%d') AND `char_pvp`.`score` > 0
		ORDER BY
			`char_pvp`.`score` DESC,
			`char`.`base_level` DESC,
			`char`.`base_exp` DESC,
