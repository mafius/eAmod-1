<?php
	...
	$who = $mysql->query("
		...
		WHERE
			`global_reg_value`.`str` = 'Mission_Rank' AND `global_reg_value`.`value` > '0' AND `login`.`level` = '0' AND `login`.`state` = '0'
		ORDER BY
			`global_reg_value`.`value` + 0 DESC
		LIMIT 0, 25
	" , $CONFIG['DBMain']);

	opentable('Ranking Hunting Missions');
?>
	<table width="600">
		<tr>
			...
			<td align="center" class="head" width="100">Misiones</td>
...
<?php
	...
	if ($who)
	{
		...
		while ($char = $mysql->fetcharray($who))
		{
			...
			if( $char['gname'] != '' )
				echo '<br>
					<span title="Ver Profile de Guild" style="color:#3dafff; cursor:pointer" onMouseOver="this.style.color=#3dafff" onMouseOut="this.style.color=#3dafff" onClick="return LINK_ajax(\'guildprofile.php?guild_id=' . $char['guild_id'] . '\',\'main_div\');">
			...
			echo '
					...
					<td align="center"><img src="./images/item/small/7773.png"> x ' . $char['value'] .'</td>
					<td align="center">
						' . $char['last_map'] . '<br>
