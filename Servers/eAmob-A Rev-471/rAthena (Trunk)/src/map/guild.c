...
void guild_guildaura_refresh(struct map_session_data *sd, int skill_num, int skill_lv) {
	...
	if( group ) {
		sc_start4(&sd->bl,(sc_type)type,100,(battle_config.guild_aura&16)?0:skill_lv,0,0,group->group_id,600000);//duration doesn't matter these status never end with val4
	}
	return;
}
