...
int unit_run(struct block_list *bl)
{
	...
	if(to_x == bl->x && to_y == bl->y) {
		...
		skill_blown(bl,bl,skill_get_blewcount(TK_RUN,lv),unit_getdir(bl),0, true);
...
int unit_skilluse_id2(struct block_list *src, int target_id, short skill_num, short skill_lv, int casttime, int castcancel)
{
	...
	if (sd) {
		...
		case GC_WEAPONCRUSH:
			if( sc && sc->data[SC_COMBO] && sc->data[SC_COMBO]->val1 == GC_WEAPONBLOCKING )
			{
				...
				{
					clif_skill_fail(sd,skill_num,USESKILL_FAIL_GC_WEAPONBLOCKING,0);
					return 0;
				}
			}
			else
	...
	if(sd) {
		if( skill_get_inf2(skill_num)&INF2_CHORUS_SKILL )
		{
			...
			{
				clif_skill_fail(sd,skill_num, USESKILL_FAIL_NEED_HELPER, 0);
				return 0;
			}
		}
		else
