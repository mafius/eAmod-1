...
/*==========================================
 * Read Database
 * ID,Type,Name,Cutin,BExp,JExp,NameID,<Parameter1>,<Count1>,...
 *------------------------------------------*/
static bool achievement_read_achievementdb(char* str[], int columns, int current)
{
	...
	for( i = 7, c = 0; i < columns && str[i]; i += 2 )
	{
		switch( ad.type )
		{
		case AT_EXPLORE:
...
void achievement_db_load(bool clear)
{
	...
	sv_readdb(db_path, "achievement_db.txt", ',', 7, 7 + (ACHIEVEMENT_OBJETIVE_MAX * 2), -1, &achievement_read_achievementdb);
}
