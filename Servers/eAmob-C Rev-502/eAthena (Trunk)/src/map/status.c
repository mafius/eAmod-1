		...
		if( sc ) {
			...
			{ // Check for Target change defenses
				...
				if( (sce = sc->data[SC_DEVOTION]) )
				{
					...
					{
						clif_damage(src, d_bl, gettick(), 0, 0, devo_hp, 0, 0, 0);
...
int status_calc_pc_(struct map_session_data* sd, bool first)
{
	...
	for(i=0;i<EQI_MAX_BONUS;i++) {
		...
		if(first && sd->inventory_data[index]->equip_script && !sd->status.inventory[index].noeffect)
		...
		if(sd->inventory_data[index]->type == IT_WEAPON) {
			...
			if(sd->inventory_data[index]->script && !sd->status.inventory[index].noeffect) {
