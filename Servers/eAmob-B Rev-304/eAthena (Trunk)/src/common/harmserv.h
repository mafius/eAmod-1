...
#define HARMSRV_VERSION_PATCH 8
...
typedef int (_TIMERCALL *HarmTimerProc)(int tid, unsigned int tick, int id, intptr_t data);
...
struct HARMSRV_EA_FUNCS {
	...
	HSRV_CALL(timer_add, int, (unsigned int tick, HarmTimerProc func, int id, intptr_t data));
	HSRV_CALL(timer_del, int, (int tid, HarmTimerProc func));

	HSRV_CALL(action_request, void, (int fd, int task, int id, intptr_t data));
