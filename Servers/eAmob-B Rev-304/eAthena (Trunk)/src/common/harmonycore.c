...
int _FASTCALL ea_timer_add(unsigned int tick, HarmTimerProc func, int id, intptr_t data);
...
int ea_timer_wrap(int tid, unsigned int tick, int id, intptr_t data) {
...
int _FASTCALL ea_timer_add(unsigned int tick, HarmTimerProc func, int id, intptr_t data) {
	...
	tid = add_timer(tick, ea_timer_wrap, 0, (intptr_t)e);
	e->data = data;
	e->id = id;
	e->func = func;
