...
BUILDIN_FUNC(itemheal)
{
	...
	if( hp && pc_isriding(sd,OPTION_MADO) )
		hp = 0; // HP Effect of Healing is 0
...
BUILDIN_FUNC(map_logincount)
{
	...
	if( sd && (m = map_mapname2mapid(var)) >= 0 )
	{
		...
		for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
		{
			...
			if( !strcmpi(s_mac_address,t_mac_address) && session[sd->fd]->client_addr == session[pl_sd->fd]->client_addr )
				i++; // Same Mac and IP = Same Computer
		}
		mapit_free(iter);
	}
