...
int pc_delitem(struct map_session_data *sd,int n,int amount,int type, short reason, e_log_pick_type log_type)
{
	...
	if (! (sd->status.inventory[n].noweight))
...
int pc_isUseitem(struct map_session_data *sd,int n)
{
	...
	noeffect = sd->status.inventory[n].noeffect;
	...
	if( item->type != IT_HEALING && item->type != IT_USABLE && item->type != IT_CASH )
		return 0;
	if( !item->script || noeffect) //if it has no script, you can't really consume it!
		return 0;
...
int pc_equipitem(struct map_session_data *sd,int n,int req_pos)
{
	...
	if (id) {
		int i;
		struct item_data *data;
		if (id->equip_script && !sd->status.inventory[n].noeffect)
		...
		for(i=0;i<id->slot; i++)
		{
			...
			if (data && data->equip_script && !sd->status.inventory[n].noeffect)
...
int pc_unequipitem(struct map_session_data *sd,int n,int flag)
{
	...
	if (sd->inventory_data[n]) {
		struct item_data *data;
		if (sd->inventory_data[n]->unequip_script && !sd->status.inventory[n].noeffect)
		...
		for(i=0;i<sd->inventory_data[n]->slot; i++)
		{
			if (!sd->status.inventory[n].card[i])
				continue;
			data = itemdb_exists(sd->status.inventory[n].card[i]);
			if (data && data->unequip_script && !sd->status.inventory[n].noeffect)
